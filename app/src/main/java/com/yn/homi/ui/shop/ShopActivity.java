...
public class ShopActivity extends BaseActivity {
    ...
    // Filter states
    private final List<String> selectedSorts = new ArrayList<>(Collections.singletonList("best_seller"));
    private final List<String> selectedCategories = new ArrayList<>(Collections.singletonList("All"));
    ...
    private TextView tvEmptySearch, tvGreeting;
    ...
    private void initViews() {
        ...
        ImageView ivFilter = findViewById(R.id.iv_menu);
    ...
    private void initFilterDrawer() {
        ...
        NavigationView navigationView = findViewById(R.id.nav_view_filter);
    ...
    private void updateTabsOnScroll(int scrollY) {
        for (int i = 0; i < shopSections.size(); i++) {
            RecyclerView.LayoutManager layoutManager = rvShopSections.getLayoutManager();
            if (layoutManager != null) {
                View sectionView = layoutManager.findViewByPosition(i);
                if (sectionView != null) {
    ...
    private void scrollToSection(int index) {
        RecyclerView.LayoutManager layoutManager = rvShopSections.getLayoutManager();
        if (layoutManager != null) {
            View sectionView = layoutManager.findViewByPosition(index);
            if (sectionView != null) {
                int scrollTo = sectionView.getTop() + rvShopSections.getTop() - dpToPx(8);
                nsvShop.smoothScrollTo(0, scrollTo);
            } else {
                // Nếu view chưa được vẽ (do lazy loading), scroll xấp xỉ hoặc dùng scroll to position
                rvShopSections.scrollToPosition(index);
                rvShopSections.post(() -> {
                    View v = layoutManager.findViewByPosition(index);
                    if (v != null) {
                        int st = v.getTop() + rvShopSections.getTop() - dpToPx(8);
                        nsvShop.smoothScrollTo(0, st);
                    }
                });
            }
        }
