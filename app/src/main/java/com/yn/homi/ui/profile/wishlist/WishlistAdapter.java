...
public class WishlistAdapter extends RecyclerView.Adapter<WishlistAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        holder.btnAddToCart.setOnClickListener(v -> {
            ...
            String defaultImageUrl = item.getImageUrl();
            if (item.getColorVariants() != null && !item.getColorVariants().isEmpty()) {
                Product.ColorVariant variant = item.getColorVariants().get(0);
                defaultColor = variant.getName();
                if (variant.getImageUrl() != null && !variant.getImageUrl().isEmpty()) {
                    defaultImageUrl = variant.getImageUrl();
                }
            }
            String defaultSize = (item.getSizeVariants() != null && !item.getSizeVariants().isEmpty())
                    ? item.getSizeVariants().get(0).getLabel() : null;
            
            CartItem cartItem = new CartItem(item.getId(), item.getName(), item.getPrice(), 1, defaultImageUrl, defaultColor, defaultSize);
